...
contract Multisig {
    ...
    uint256 public quorumUpdateCount; // Counter for quorum update proposals
    ...
    struct QuorumUpdate {
        ...
        bool isCompleted;
        uint256 noOfApproval;
        address[] quorumUpdateSigners;
    }

    mapping(uint256 => QuorumUpdate) public quorumUpdates; // Stores quorum update proposals
    ...
    function proposeQuorumUpdate(uint8 _newQuorum) external {
        require(msg.sender != address(0), "address zero found");
        require(isValidSigner[msg.sender], "invalid signer");
        require(_newQuorum > 1 && _newQuorum <= noOfValidSigners, "invalid quorum size");

        uint256 _updateId = quorumUpdateCount + 1;
        QuorumUpdate storage qUpdate = quorumUpdates[_updateId];

        qUpdate.id = _updateId;
        qUpdate.newQuorum = _newQuorum;
        qUpdate.noOfApproval += 1;
        qUpdate.quorumUpdateSigners.push(msg.sender);
        hasSignedQuorumUpdate[msg.sender][_updateId] = true;

        quorumUpdateCount += 1;
    }

    function approveQuorumUpdate(uint256 _updateId) external {
        QuorumUpdate storage qUpdate = quorumUpdates[_updateId];

        require(qUpdate.id != 0, "invalid update id");
        require(!qUpdate.isCompleted, "quorum update already completed");
        require(qUpdate.noOfApproval < quorum, "approvals already reached");
        require(isValidSigner[msg.sender], "not a valid signer");
        require(!hasSignedQuorumUpdate[msg.sender][_updateId], "can't sign twice");
